...
export class UssdMenuService {
  ...
  private async matches(session: UssdSession): Promise<UssdScreen> { const matches = (await buildRankedFeed(this.repository, session.userId!)).slice(0, USSD_RULES.maxMatches); session.matchIds = matches.map((item) => item.opportunityId); session.step = "matches"; const lines = matches.map((item, index) => `${index + 1} ${item.opportunity?.title} - ${item.matchedFactors[0]?.detail} ${item.opportunity?.deadline ? `Closes ${item.opportunity.deadline.toLocaleDateString("en-UG")}` : "Rolling"}`); return this.persist(session, screen(lines.length ? `${lines.join("\n")}\n0 Back` : "No new verified matches.\n0 Back")); }
  ...
  private async deadlines(session: UssdSession): Promise<UssdScreen> { const saved = await this.repository.listSaved(session.userId!); const items = (await Promise.all(saved.filter((item) => item.status === "saved").map((item) => this.repository.getOpportunity(item.opportunityId)))).filter((item) => item !== null).sort((a, b) => (a.deadline?.getTime() ?? Number.POSITIVE_INFINITY) - (b.deadline?.getTime() ?? Number.POSITIVE_INFINITY)).slice(0, 3); session.step = "deadlines"; return this.persist(session, screen(items.length ? `${items.map((item) => `${item.title}: ${item.deadline?.toLocaleDateString("en-UG") ?? "Rolling; no closing date"}`).join("\n")}\n0 Back` : "No saved opportunities.\n0 Back")); }
